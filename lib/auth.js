 ...
 * This function is read-only so it can safely run during page rendering.
 */
      ...
      if (refreshPayload?.userId) {
        ...
        if (storedToken && storedToken.expiresAt > new Date()) {
          ...
          if (user && user.isActive) {
            const activeDeptCookie = cookieStore.get("sf_active_dept")?.value;
            const validCookie =
              activeDeptCookie &&
              (user.role === "ADMIN" ||
                user.memberships?.some((m) => m.departmentId === activeDeptCookie));

            user.activeDepartmentId = validCookie
              ? activeDeptCookie
              : user.departmentId || user.memberships?.[0]?.departmentId || null;
